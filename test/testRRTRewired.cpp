...
void testRRTX(){
  ...
  //std::cerr<<"1"<<std::endl;
  RRTX_.initilize(env,q_init,q_goal,accuracy,upper,lower,10000,5.0);
...
void testRRTXRandom(){
  ...
  gidisizi::RRTXRandom<NodeType_,Environment_> RRTX_ = gidisizi::RRTXRandom<NodeType_,Environment_>();
  int nodeId = 0;
  NodeType_* q_init = new NodeType_(nodeId++, Eigen::Vector2d(-0.7,0.8));
  NodeType_* q_goal = new NodeType_(nodeId++, Eigen::Vector2d(0.8,-0.8));
  Eigen::VectorXd upper = Eigen::VectorXd::Ones(2);
